...
string toStr(ll a){ostringstream oss; oss<<a; return oss.str();}
int toInt(string s){return atoi(s.substr(0,s.size()).c_str());}


const ll MAX = 10000000005LL;
...
public:
  int minimalChange(string ID) {
    ...
    int ans = ID.length();
    for(int i = 0; i < (int)ID.length(); i++) if(ID[i] == '0') ans--;
    
    for(ll i = 1; i*i < MAX; i++){
      string tmp = toStr(i*i);
      while(tmp.length() < ID.length()) tmp = "0"+tmp;
      if(tmp.length() != ID.length()) break;
      int cnt = 0;
      for(int j = 0; j < (int)ID.length(); j++) if(ID[j] != tmp[j]) cnt++;
      ans = min(ans, cnt);
    }
    return ans;
  }
