	...
	double getAverage(vector <string> ratings) {
	  ...
	  for(int i = 0; i < N; i+=R){
	    ...
	    for(int j = i; j < min(i+R,N); j++){
	      if(v[j] == Elly) hasElly = true;
	      tmp += v[j];
	      added++;
	    }
	    if(hasElly) aveRoomRate.push_back(Elly);
	    else aveRoomRate.push_back(tmp/added);
	  }
	
	  
	  int Ellypos = 0;
	  for(int i = 0; i < N; i++)
	    if(v[i] == Elly) Ellypos = i;
	  
	  double sum = 0;
	  for(int i = 0; i < aveRoomRate.size(); i++)
	    sum+=aveRoomRate[i];
	  ...
	  if(N%R == 0){
	    ...
	    double p0 = 1.0-p1;
	    res = p1*(sum/(T+1))+p0*(sum-aveRoomRate[T])/T;
	  }
	  return res;
	  
	}

  vector<int> make(const vector<string>& v){
    vector<int> res;
    string s = "";
