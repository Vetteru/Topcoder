...
public:
  int maxProfit(vector <int> cityValues, vector <string> roads) {
    ...
    while(!Q.empty()){
      ...
      if(come[p.first][p.second]) continue;
      come[p.first][p.second] = true;
      ans = max(ans, p.second);

      for(int i = 0; i < (int)roads[p.first].size(); i++){
	if(roads[p.first][i] == 'N') continue;
	P nex = p;
	nex.first = i;
	nex.second = p.second^cityValues[i];
	if(!come[nex.first][nex.second]) Q.push(nex);
      }
    }
    return ans;
  }
