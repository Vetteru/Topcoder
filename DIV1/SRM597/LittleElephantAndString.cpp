...
public:
  int getNumber(string A, string B) {
    if(impossible(A,B)) return -1;
    for(int i = 0; i < (int)B.length(); i++)
      if(isContain(B.substr(i),A)) return i;
    return A.length();
  }

  bool isContain(const string& A, const string& B){
    int pos = 0;
    for(int i = 0; i< (int)A.length(); i++){
      while(pos < (int)B.length() && A[i] != B[pos]) pos++;
      if(pos >= (int)B.length()) return false;
      pos++;
    }
    return true;
  }

  bool impossible(string A, string B){
    sort(A.begin(),A.end());
    sort(B.begin(),B.end());
    return A!=B;
  }
