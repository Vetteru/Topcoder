...
public:
  int getmax(vector <int> D, int X) {
    ...
    for(int i = 0; i < N; i++)
      for(ITE ite = dp[i].begin(); ite != dp[i].end(); ite++){
	int val = ite->first;
	
	if(val >= 2200){
	  int nex = max(0,val-D[i]);
	  if(nex >= 2200) continue;
	  dp[i+1][nex] = max(dp[i+1][nex], ite->second + ((val < 2200)!=(nex < 2200)?1:0));
	}else{
	  for(int s = 0; s < 2; s++){
	    int nex = (s==0)?val+D[i]:max(0,val-D[i]);
	    dp[i+1][nex] = max(dp[i+1][nex], ite->second + ((val < 2200)!=(nex < 2200)?1:0));
	  }
	}
      }
    ...
    int ans = 0;
    for(ITE ite = dp[N].begin(); ite != dp[N].end(); ite++) ans = max(ans, ite->second);
    return ans;
  }
