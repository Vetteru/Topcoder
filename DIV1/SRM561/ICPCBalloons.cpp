...
public:
  int minRepaintings(vector <int> balloonCount, string balloonSize, vector <int> maxAccepted) {
    ...
    vector<int> large, medium;
    for(int i = 0; i < N; i++){
      if(balloonSize[i] == 'L') large.push_back(balloonCount[i]);
      else medium.push_back(balloonCount[i]);
    }

    int p = maxAccepted.size();
    int ans = -1;
    for(int i = 0; i < (1<<p); i++){
      vector<int> A,B;
      for(int j = 0; j < p; j++){
	if(i & (1<<j)) A.push_back(maxAccepted[j]);
	else B.push_back(maxAccepted[j]);
      }
  ...
  int calc(vector<int> prob, vector<int> ball){
    sort(prob.begin(),prob.end());
    reverse(prob.begin(),prob.end());
    sort(ball.begin(),ball.end());
    reverse(ball.begin(),ball.end());

    while(prob.size() > ball.size()) ball.push_back(0);

    int remain = 0, res = 0;
    for(int i = (int)prob.size(); i < (int)ball.size(); i++) remain += ball[i];
    for(int i = 0; i < (int)prob.size(); i++){
      if(prob[i] <= ball[i]){
	remain += ball[i]-prob[i];
      }else{
	remain -= prob[i]-ball[i];
	res += prob[i]-ball[i];
      }
    }
