...
public:
  ...
  int maxDifference(vector <string> isFriend, int d) {
    ...
    fill(len[0],len[0]+MAX*MAX,INF);
    
    for(int i = 0; i< MAX; i++) len[i][i] = 0;

    for(int i = 0; i < isFriend.size(); i++)
      for(int j = 0; j < isFriend[i].size(); j++)
	if(isFriend[i][j] == 'Y') len[i][j] = 1;
    ...
    */
    ...
    if(!isValid(isFriend.size())) return -1;
  ...
  bool isValid(int n){
    for(int i = 0; i < n; i++)
      for(int j = 0; j < n; j++)
	if(i != j && len[i][j] == INF) return false;
    return true;
  }
