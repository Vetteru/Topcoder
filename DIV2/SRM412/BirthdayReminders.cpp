...
struct Info{
  ...
  bool operator < (const Info& info) const {
    if(day != info.day) return day < info.day;
    if(oc != info.oc) return oc < info.oc;
    return fre < info.fre;
  }
};
...
public:
  vector <string> remind(vector <string> friendNames, vector <int> birthDates, int currentDate,
			 vector <string> occasions, vector <int> days, int k) {

    priority_queue<Info> Q;
    int N = friendNames.size();
    int M = occasions.size();
    for(int i = 0; i < N; i++)
      for(int j = 0; j < M; j++){
	int x = (currentDate-birthDates[i])/days[j];
	while(birthDates[i] + days[j]*x < currentDate) x++;
	int s = birthDates[i] + days[j]*x;
	for(int cnt = 0; cnt < 100; cnt++){
	  string event = toStr(s) + ". " + friendNames[i] + " " +  occasions[j] + " (" + toStr(x) + ")";
	  Q.push((Info){i,j,s,event});
